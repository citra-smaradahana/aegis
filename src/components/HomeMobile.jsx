...
function HomeMobile({ user, onNavigate, validationCount = 0, ftwNeedsFill = false, tasklistTodoCount = 0 }) {
  ...
  const allMenuItems = [
    ...
    ...(canAccessMonitoring
      ? [{ key: "monitoring", label: "Monitoring", fullLabel: "Monitoring", icon: "📊", color: "#ea580c", placeholder: false }]
      : []),
  ...
  const menuItemsRaw = (allowedMenus
    ? allMenuItems.filter((item) => !item.placeholder && (allowedMenus.includes(item.key) || (item.key === "monitoring" && canAccessMonitoring)))
