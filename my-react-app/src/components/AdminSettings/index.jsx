  ...
  const loadParentItems = async () => {
    if (activeType === "site_location") {
      ...
      setSelectedParentId((prev) => {
        ...
        return prev && data.some((d) => d.id === prev)
          ? prev
          : data[0]?.id || null;
      });
      ...
      setItems(siteData);
      if (!menuJabatanId && jabatanData.length)
        setMenuJabatanId(jabatanData[0].id);
  ...
  const loadItems = async () => {
    ...
    try {
      if (activeType === "site") {
        ...
        if (selectedParentId) {
          const data =
            await fetchProsedurForAdminByDepartemenId(selectedParentId);
          setItems(data);
        } else {
          setItems([]);
        }
        ...
        if (menuSubTab === "jabatan" && menuJabatanId) {
          ...
          if (!cancelled) {
            ...
            const { data: jRow } = await supabase
              ...
              .eq("name", jabatanName)
              .maybeSingle();
            const jabatanKeys = jRow?.id
              ? await fetchJabatanMenus(jRow.id)
              : [];
            setMenuJabatanDefault(jabatanKeys);
            setMenuSelectedKeys(
              overrideKeys.length > 0 ? overrideKeys : jabatanKeys,
            );
          }
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (activeType === "site") {
        ...
        if (editingItem)
          await updateProsedurDepartemen(editingItem.id, formName);
        ...
        if (editingItem) await updateAlasanObservasi(editingItem.id, formName);
                ...
                {["jabatan", "site", "user"].map((tab) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: "12px 16px",
                      background:
                        menuSubTab === tab ? "#3b82f6" : "transparent",
              ...
              >
                      ...
                      {MENU_KEYS.map((m) => (
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked)
                                setMenuSelectedKeys((prev) => [...prev, m.key]);
                              else
                                setMenuSelectedKeys((prev) =>
                ...
                {menuSubTab === "site" && (
                  <>
                    ...
                    >
                      Centang menu yang tersedia untuk site ini. Kosongkan =
                      semua menu dari jabatan/user tetap tampil.
                    </p>
                    <div
                      style={{
                        display: "flex",
                    ...
                    >
                      {MENU_KEYS.map((m) => {
                        const item = menuSiteItems.find(
                          (x) => x.menu_key === m.key,
                        ) || { menu_key: m.key, enabled: true };
                        return (
                          <label
                            key={m.key}
                            style={{
                ...
                {menuSubTab === "user" && (
                  <>
                    ...
                    >
                      Filter Site dan Jabatan, lalu pilih user untuk mengubah
                      menu tambahan (override).
                    </p>
                    <div
                      style={{
                        display: "flex",
                    ...
                    >
                      <div style={{ flex: "1 1 200px" }}>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setMenuUserSiteFilter(e.target.value)
                          }
                          style={{
                            width: "100%",
                            padding: "10px 14px",
                        ...
                        >
                          ...
                          {[
                            ...new Set(
                              menuUsers.map((u) => u.site).filter(Boolean),
                            ),
                          ]
                            .sort()
                            .map((s) => (
                      ...
                      <div style={{ flex: "1 1 220px" }}>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setMenuUserJabatanFilter(e.target.value)
                          }
                          style={{
                            width: "100%",
                            padding: "10px 14px",
                        ...
                        >
                          ...
                          {[
                            ...new Set(
                              menuUsers.map((u) => u.jabatan).filter(Boolean),
                            ),
                          ]
                            .sort()
                            .map((j) => (
                        ...
                        <tbody>
                          {menuUsers
                            .filter(
                              (u) =>
                                (!menuUserSiteFilter ||
                                  u.site === menuUserSiteFilter) &&
                                (!menuUserJabatanFilter ||
                                  u.jabatan === menuUserJabatanFilter),
                            )
                            .map((u) => (
                              <tr
                                key={u.id}
                    ...
                    {menuUserEditModal && (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {menuUserEditModal.site} ·{" "}
                            {menuUserEditModal.jabatan}
                          </p>
                          <p
                            style={{
                              color: "#9ca3af",
                          ...
                          >
                            Default jabatan:{" "}
                            {menuJabatanDefault.join(", ") || "(belum diatur)"}
                          </p>
                          <p
                            style={{
                              color: "#6b7280",
                          ...
                          >
                            Centang menu yang boleh diakses user. Bisa menambah
                            atau menghapus dari default jabatan (untuk site lain
                            nanti).
                          </p>
                          <div
                            style={{
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  {isFTWSettings && (
                    <span style={{ color: "#6b7280", fontSize: 12 }}>
                      Toggle OFF = site tidak wajib FTW (user tetap boleh
                      mengisi)
                    </span>
                  )}
                  {canImport && (
                          ...
                          >
                            ...
                            >
                              <button
                                ...
                                style={{
                                  padding: "6px 14px",
                                  background: item.fit_to_work_enabled
                                    ? "#22c55e"
                                    : "#6b7280",
                                  color: "#fff",
                                  border: "none",
