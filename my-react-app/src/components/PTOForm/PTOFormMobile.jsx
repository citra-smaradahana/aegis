...
function PTOFormMobile({
  ...
  const [selectedProsedurDepartemenName, setSelectedProsedurDepartemenName] =
    useState("");
  const [selectedProsedurDepartemenId, setSelectedProsedurDepartemenId] =
    useState(null);
  const [selectedProsedurId, setSelectedProsedurId] = useState(null);
  ...
  const [showProsedurDepartemenModal, setShowProsedurDepartemenModal] =
    useState(false);
  const [personSearch, setPersonSearch] = useState("");
  const [prosedurDepartemenSearchQuery, setProsedurDepartemenSearchQuery] =
    useState("");
  ...
  useEffect(() => {
    ...
    getLocationOptionsAsync(formData.site).then((opts) => {
      if (!cancelled)
        setDetailLokasiOptions(opts || getLocationOptions(formData.site) || []);
    });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
      const [alasan, dept] = await Promise.all([
        fetchAlasanObservasi(),
        fetchProsedurDepartemen(),
      ]);
      if (cancelled) return;
      ...
      const prosedur = await fetchProsedur(
        selectedProsedurDepartemenId || null,
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const ptoData = {
        ...
        observer_id: user.id,
        nama_observer: user.nama, // Kirim nama observer
        nrp_pelapor: user.nrp, // Kirim NRP pelapor secara eksplisit
        jabatan_pelapor: user.jabatan, // Kirim Jabatan pelapor
        ...
        prosedur_departemen_id: selectedProsedurDepartemenId || null,
        prosedur_id: selectedProsedurId || null,
      ...
      if (
        ...
        await supabase.rpc("create_hazard_report_from_pto", {
          pto_id: data[0].id,
        });
      }
      setSuccess(true);
  ...
  return (
    <div style={contentStyle}>
      ...
      >
        ...
        <button
          onClick={() => onSwitchToHistory && onSwitchToHistory()}
          style={{
            flex: 1,
            padding: 8,
            borderRadius: 8,
      ...
      <div style={scrollContentStyle}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <>
                    ...
                    >
                      <span
                        style={{
                          color:
                            formData.detailLokasi || showCustomDetailInput
                              ? "#222"
                              : "#6b7280",
                        }}
                      >
              ...
              <div style={fieldMargin}>
                ...
                >
                  <span
                    style={{
                      color: formData.alasanObservasi ? "#222" : "#6b7280",
                    }}
                  >
              ...
              <div style={fieldMargin}>
                ...
                <div
                  onClick={() => {
                    ...
                    setShowProsedurDepartemenModal(true);
                  }}
                  style={tapFieldStyle}
                >
                  <span
                    style={{
                      color: selectedProsedurDepartemenName
                        ? "#222"
                        : "#6b7280",
                    }}
                  >
                    {selectedProsedurDepartemenName ||
              ...
              <div style={fieldMargin}>
                ...
                <div
                  ...
                  style={{
                    ...tapFieldStyle,
                    background: !selectedProsedurDepartemenId
                      ? "#f3f4f6"
                      : undefined,
                    color: !selectedProsedurDepartemenId
                      ? "#9ca3af"
                      : undefined,
                    cursor: !selectedProsedurDepartemenId
                      ? "not-allowed"
                      : "pointer",
                  }}
                >
                  <span
                    style={{ color: formData.prosedur ? "#222" : "#6b7280" }}
                  >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={fieldMargin}>
                ...
                >
                  <span
                    style={{
                      color: formData.observerTambahan ? "#222" : "#6b7280",
                    }}
                  >
                    {getPersonLabel(formData.observerTambahan, observers) ||
                      "Pilih Observer Tambahan"}
                  </span>
                  <svg
                    width="16"
                    height="16"
              ...
              <div style={fieldMargin}>
                ...
                <div
                  onClick={() => {
                    setPersonSearch("");
                    setShowObserveeModal(true);
                  }}
                  style={tapFieldStyle}
                >
                  <span
                    style={{ color: formData.observee ? "#222" : "#6b7280" }}
                  >
                    {getPersonLabel(formData.observee, observees) ||
                      "Pilih Observee"}
                  </span>
                  <svg
                    width="16"
              ...
              {[
                {
                  ...
                  q: "Apakah langkah kerja aman dilakukan?",
                },
                {
                  key: "apdSesuai",
                  q: "Apakah pekerja menggunakan APD yang sesuai?",
                },
                { key: "areaKerjaAman", q: "Apakah area kerja aman?" },
                {
                  key: "peralatanAman",
                  q: "Apakah peralatan yang digunakan telah sesuai dan aman?",
                },
                {
                  key: "peduliKeselamatan",
                  q: "Apakah pekerja peduli dengan keselamatan rekan kerjanya?",
                },
                {
                  key: "pahamResikoProsedur",
                  q: "Apakah pekerja memahami resiko pekerjaan dan prosedur kerjanya?",
                },
              ].map(({ key, q }) => (
                <div key={key} style={fieldMargin}>
                  ...
                  >
                    <button
                      ...
                      style={checklistBtn(formData[key] === true, "#22c55e")}
                    >
                      Iya
                    </button>
                    <button
                      ...
                      style={checklistBtn(formData[key] === false, "#ef4444")}
                    >
                      Tidak
                    </button>
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={fieldMargin}>
                ...
                >
                  <span
                    style={{
                      color: formData.picTindakLanjut ? "#222" : "#6b7280",
                    }}
                  >
                    {getPersonLabel(formData.picTindakLanjut, pics) ||
                      "Pilih PIC Tindak Lanjut"}
                  </span>
                  <svg
                    width="16"
                    height="16"
                ...
                {!formData.fotoTemuan ? (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setFormData((p) => ({ ...p, fotoTemuan: null }))
                      }
                      style={{
                        padding: "8px 16px",
                        background: "#ef4444",
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                {page < 4 ? (
                  ...
                  <button
                    ...
                    style={{
                      background:
                        submitting || !validatePage() ? "#9ca3af" : "#2563eb",
                      color: "#fff",
                      border: "none",
                      borderRadius: 8,
                      padding: "8px 0",
        ...
        <SelectModalWithSearch
          ...
          onSelect={async (val) => {
            ...
            const found = (prosedurDepartemenList || []).find(
              (d) => d.name === val,
            );
        ...
        <SelectModalWithSearch
          ...
          options={["Tidak ada", ...(prosedurOptions || []).map((p) => p.name)]}
          onSelect={(val) => {
            if (val === "Tidak ada") {
              setFormData((p) => ({ ...p, prosedur: "" }));
              setSelectedProsedurId(null);
            } else {
              const found = (prosedurOptions || []).find((p) => p.name === val);
              setFormData((p) => ({ ...p, prosedur: val }));
              setSelectedProsedurId(found?.id || null);
            }
        ...
        <PersonSelectModal
          ...
          onSelect={(id) => {
            setFormData((p) => ({ ...p, observerTambahan: id || "" }));
        ...
        <PersonSelectModal
          ...
          onSelect={(id) => {
            setFormData((p) => ({ ...p, observee: id || "" }));
            setShowObserveeModal(false);
          }}
          searchQuery={personSearch}
          onSearchChange={setPersonSearch}
        />
        <PersonSelectModal
          ...
          onSelect={(id) => {
            setFormData((p) => ({ ...p, picTindakLanjut: id || "" }));
