...
WITH ftw_today AS (
  ...
  FROM fit_to_work f WHERE f.tanggal IS NOT NULL
),
ftw_yesterday AS (
  SELECT f.nrp, f.site, f.tanggal, f.total_jam_tidur AS sleep_yesterday
  FROM fit_to_work f WHERE f.tanggal IS NOT NULL
),
combined AS (
  SELECT u.id AS user_id, u.nama, u.jabatan, u.nrp, u.site, t.tanggal,
    ...
    t.tidak_mengkonsumsi_obat, t.tidak_ada_masalah_pribadi, t.siap_bekerja
  FROM users u
  INNER JOIN ftw_today t ON u.nrp = t.nrp AND u.site = t.site
  LEFT JOIN fit_to_work_attendance_summary a ON u.id = a.user_id
  LEFT JOIN ftw_yesterday y ON u.nrp = y.nrp AND u.site = y.site AND y.tanggal = t.tanggal - 1
)
SELECT user_id, nama, jabatan, nrp, site, tanggal, hari_masuk, sleep_today, sleep_48h,
  tidak_mengkonsumsi_obat, tidak_ada_masalah_pribadi, siap_bekerja
FROM combined;
...
RETURNS TABLE (user_id UUID, nama TEXT, jabatan TEXT, nrp TEXT, hari_masuk INT,
  sleep_today NUMERIC, sleep_48h NUMERIC, tidak_mengkonsumsi_obat BOOLEAN,
  tidak_ada_masalah_pribadi BOOLEAN, siap_bekerja BOOLEAN) AS $$
BEGIN
  RETURN QUERY
  SELECT v.user_id, v.nama, v.jabatan, v.nrp, v.hari_masuk::INT, v.sleep_today, v.sleep_48h,
    v.tidak_mengkonsumsi_obat, v.tidak_ada_masalah_pribadi, v.siap_bekerja
  FROM v_daily_attendance_ftw v
  WHERE v.tanggal = p_tanggal AND v.site = p_site
